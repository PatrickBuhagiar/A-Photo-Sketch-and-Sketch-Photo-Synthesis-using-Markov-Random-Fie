...
for ps = 1:n_ps,
    ...
    for ol = 2: n_ol,
        overlap_size = patch_sizes(ol, ps);
            ...
            K
            %% acquire patches
            [Training_Sketch_Patches, Training_Sketch_Warped] = InitializeTrainingSet(train_sketch_path, sketch_list, model_path, n_training_sketches, patch_size, overlap_size);
            [Training_Photo_Patches, Training_Photo_Warped] = InitializeTrainingSet(train_photo_path, image_list, model_path, n__training_photos,  patch_size, overlap_size);
            ...
            for t_i=1:n_photo_input, %for each training image
                ...
                %% Generate Image
                I_A = GeneratePsuedoPhoto(candidate_patches_A, full_image_size, overlap_size);
                I_B = GeneratePsuedoPhoto(candidate_patches_B, full_image_size, overlap_size);
                %figure; imshow(I_A); figure; imshow(I_B);
                mkdir(['no_candidates_images_A_' num2str(patch_size) '_' num2str(overlap_size) '_' num2str(K) ]);
                mkdir(['no_candidates_images_B_' num2str(patch_size) '_' num2str(overlap_size) '_' num2str(K) ]);
                I_A_file = ['no_candidates_images_A_' num2str(patch_size) '_' num2str(overlap_size) '_' num2str(K) '\I_A_'  num2str(t_i) '.jpg'];
                I_B_file = ['no_candidates_images_B_' num2str(patch_size) '_' num2str(overlap_size) '_' num2str(K) '\I_B_'  num2str(t_i) '.jpg'];
            ...
            %% csv write
            ssim_A_file = ['results\no_candidates_ssim_A_' num2str(patch_size) '_' num2str(overlap_size) '_' num2str(K) 'I_A_'  num2str(t_i) '.csv'];
            ssim_B_file = ['results\no_candidates_ssim_B_' num2str(patch_size) '_' num2str(overlap_size) '_' num2str(K) 'I_B_'  num2str(t_i) '.csv'];
            psnr_A_file = ['results\no_candidates_psnr_A_' num2str(patch_size) '_' num2str(overlap_size) '_' num2str(K) 'I_A_'  num2str(t_i) '.csv'];
            psnr_B_file = ['results\no_candidates_psnr_B_' num2str(patch_size) '_' num2str(overlap_size) '_' num2str(K) 'I_B_'  num2str(t_i) '.csv'];
